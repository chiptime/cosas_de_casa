...
void movimiento_Bola() {
    ...
    if((xbola<=5) || (xbola>=99)) ////hacer otras dos implicaciones logicas (_) son 3 espacios
        xdireccion *= -1;
    if((ybola<=3) || (ybola>=29))
        ydireccion *= -1;
    //contra las barras
    ...
    gotoxy(55,10);printf("%c", loquefalta);            // situacion de la bola
}
void teclas(){
...
close(STDIN_FILENO);
read(STDIN_FILENO,mygetch(),flecha);
