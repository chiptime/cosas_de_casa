...
CREATE TABLE AEROPUERTO(codIATA CHAR(3), nombre VARCHAR(30), ciudad VARCHAR(30), pais VARCHAR(30), PRIMARY KEY (codIATA,nombre,ciudad));
...
CREATE TABLE /*BILLETE-> agregar otras columnas como  etc*/RESERVA_VUELO(localizador INT, codvuelo INT, DNI CHAR(3), nombre VARCHAR(30), maleta_mano ENUM('si','no'), maleta_bodega ENUM('0','1','2','3') CONSTRAINT local_FK FOREIGN KEY (localizador) REFERENCES RESERVA(localizador), CONSTRAINT cod_vuelo_FK FOREIGN KEY (codvuelo) REFERENCES VUELO(codvuelo));
